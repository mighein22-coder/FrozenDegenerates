  ...
  return (
    <div className={`group relative bg-slate-900 border border-slate-800 rounded-lg p-3 transition-all duration-300 ${isSelected ? 'ring-1 ring-ice-400/40 bg-slate-900/80' : 'hover:border-slate-700'}`}>
      ...
      <div className="flex items-center justify-between mb-2 relative z-20">
        ...
        <button 
          onClick={handleAskAi}
          className={`text-[10px] flex items-center gap-1 transition-colors ${disabled ? 'hidden' : 'text-ice-500 hover:text-ice-400'}`}
        >
          <Brain size={12} />
          {loadingAi ? '...' : 'AI'}
        </button>
      ...
      {/* AI Analysis Result */}
      {aiAnalysis && (
        <div className="mt-2 p-2 bg-ice-900/40 border border-ice-500/20 rounded text-xs text-ice-100 leading-snug animate-in fade-in slide-in-from-top-1 relative z-20">
          {aiAnalysis}
        </div>
      )}
    </div>
